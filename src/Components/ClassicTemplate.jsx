  ...
  return (
    <div style={{ fontFamily: "'Times New Roman', serif", background: "#fff", color: "#1a1a1a", minHeight: "100%", fontSize: "10px", padding: "28px 32px" }}>
      <div style={{ textAlign: "center", borderBottom: `2px solid ${t.accent}`, paddingBottom: "14px", marginBottom: "16px" }}>
        <div style={{ fontSize: "24px", fontWeight: "900", letterSpacing: "3px", textTransform: "uppercase" }}>{data.name || "YOUR NAME"}</div>
        <div style={{ fontSize: "11px", color: t.accent, marginTop: "4px", letterSpacing: "1px" }}>{data.title}</div>
      ...
      {data.summary && <><ClassicSectionTitle title="Objective" accent={t.accent}/><p style={{ lineHeight: "1.7", marginBottom: "14px" }}>{data.summary}</p></>}
        ...
        {data.experience.filter(e => e.company).map(e => (
          <div key={e.id} style={{ marginBottom: "12px" }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <strong>{e.role}, {e.company}</strong><span style={{ color: "#888" }}>{e.period}</span>
            </div>
      ...
      {data.education?.some(e => e.school) && (
        ...
        {data.education.filter(e => e.school).map(e => (
          <div key={e.id} style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
            <span><strong>{e.degree}</strong>, {e.school}</span><span style={{ color: "#888" }}>{e.year}</span>
          </div>
        ))}</>
      )}
      {data.skills?.some(s => s) && (
        <><ClassicSectionTitle title="Skills" accent={t.accent}/><p>{data.skills.filter(s => s).join(" · ")}</p></>
      )}
    </div>
  );
}
