  ...
  return (
    <div style={{ fontFamily: "'Georgia', serif", background: t.bg, color: t.text, minHeight: "100%", fontSize: "10px" }}>
      ...
      <div style={{ background: t.sidebar, color: "#fff", padding: "24px 28px 20px" }}>
        ...
        <div style={{ fontSize: "12px", color: t.accent, marginTop: "3px", fontStyle: "italic" }}>{data.title || "Your Title"}</div>
      ...
      <div style={{ padding: "20px 28px" }}>
            ...
            {data.experience.filter(e => e.company).map(e => (
              <div key={e.id} style={{ marginBottom: "14px" }}>
                ...
                <ul style={{ margin: "0", paddingLeft: "14px" }}>
                  {e.bullets.filter(b => b).map((b, i) => <li key={i} style={{ marginBottom: "2px", lineHeight: "1.5" }}>{b}</li>)}
                </ul>
              </div>
            ))}
          </Section>
            ...
            <Section title="Education" accent={t.accent}>
              {data.education.filter(e => e.school).map(e => (
                <div key={e.id} style={{ marginBottom: "8px" }}>
                  ...
                  <div style={{ color: "#94a3b8", fontSize: "9px" }}>{e.year}{e.gpa ? ` · GPA: ${e.gpa}` : ""}</div>
                </div>
              ))}
            </Section>
          )}
        ...
        {data.certifications?.some(c => c) && (
          <Section title="Certifications" accent={t.accent}>
            {data.certifications.filter(c => c).map((c, i) => <div key={i} style={{ marginBottom: "3px" }}>• {c}</div>)}
          </Section>
        )}
        {data.languages?.some(l => l) && (
          <Section title="Languages" accent={t.accent}>
            <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
              {data.languages.filter(l => l).map((l, i) => <span key={i}>• {l}</span>)}
            </div>
          </Section>
        )}
      </div>
